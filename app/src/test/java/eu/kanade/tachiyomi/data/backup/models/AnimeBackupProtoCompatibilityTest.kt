...
class AnimeBackupProtoCompatibilityTest {
    ...
    fun `legacy backup bytes decode with empty anime payload`() {
        ...
        ProtoBuf.decodeFromByteArray(Backup.serializer(), bytes).backupAnime shouldBe emptyList()
    }
    ...
    fun `legacy profile scoped backup bytes decode with empty anime payload`() {
        ...
        ProtoBuf.decodeFromByteArray(ProfileScopedBackup.serializer(), bytes).anime shouldBe emptyList()
    }
